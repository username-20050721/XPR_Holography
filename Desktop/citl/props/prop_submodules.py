...
class Field2Input(nn.Module):
    ...
    def __init__(self, input_res=(800, 1280), coord='rect', latent_amp=None, latent_phase=None, shared_cnn=False):
...
class Conv2dField(nn.Module):
    ...
    def __init__(self, complex=False, conv_size=3):
        super(Conv2dField, self).__init__()
        self.complex = complex  # apply convolution on field
    ...
    def forward(self, input_field):
        # check if input is light field
        if len(input_field.shape) > 4:
            lf_batch_size = input_field.shape[0]
            num_ch = input_field.shape[1]
            num_y = input_field.shape[4]
            num_x = input_field.shape[5]
            input_field = input_field.permute(0, 4, 5, 1, 2, 3)
            input_field = input_field.reshape(lf_batch_size * num_y * num_x, num_ch, *input_field.shape[-2:])
            lf = True
        else:
            lf = False
        ...
        if lf:
            output_field = output_field.reshape(lf_batch_size, num_y, num_x, num_ch, *output_field.shape[-2:])
            output_field = output_field.permute(0, 3, 4, 5, 1, 2)
    ...
    def forward(self, phases, latent_codes=None):
        ...
        for i in range(len(self.num_latent_codes)):
            if latent_codes is not None:
                latent_codes_b = latent_codes.repeat(phases.shape[0], 1, 1, 1)
                after_relu = torch.cat((after_relu, latent_codes_b[:, self.idxs[i]:self.idxs[i + 1], ...]), 1)
...
class FiniteDiffField(nn.Module):
    def __init__(self):
        super(FiniteDiffField, self).__init__()
        pass
    
    def forward(self, model, slm_phase, delta_phase):
        # delta phase is the phase difference to be added to the input phase. 
        # Can sample some SLM locations each iteration
        
        field_1 = model(slm_phase)
        field_2 = model(slm_phase + delta_phase) 
        # size H*W, which is the ith column of Jacobian df/d(phi)
        delta_field = (field_2 - field_1) / delta_phase 
        return delta_field
